...
(() => {
    ...
    // ── Export toast (top-right, non-blocking) ─────────────────────
    // Shows where a long export is up to, the outcome, and a ✕ to cancel.
    let toastEl = null;
    let toastTimer = null;
    let toastText = null;
    let toastClose = null;
    ...
    // The export toast keeps its own clock. A counter that has not moved for a
    // while is ambiguous — a slow service and a dead one look identical — so
    // the elapsed time ticks locally, and the toast says outright how long it
    // has been since it last heard anything.
    const EXPORT_STALE_MS = 20000;
    const EXPORT_SILENCE_MS = 120000;
    let exportBaseText = 'Exporting';
    let exportNote = '';
    let exportStartedAt = 0;
    let exportLastUpdate = 0;
    ...
    function renderExportToast() {
        const now = Date.now();
        const quiet = now - exportLastUpdate;
        let text = exportBaseText;
        if (exportStartedAt) text += ` · ${formatDuration(now - exportStartedAt)}`;
        if (exportNote) text += ` — ${exportNote}`;
        else if (quiet > EXPORT_STALE_MS) text += ` — no update for ${Math.round(quiet / 1000)}s`;
        showToast('loading', { text, cancellable: true });
    }

    function noteExportActivity() {
        exportLastUpdate = Date.now();
        if (!exportStartedAt) exportStartedAt = exportLastUpdate;
        startExportPolling();
        if (exportTicker) return;
        exportTicker = setInterval(() => {
            if (Date.now() - exportLastUpdate > EXPORT_SILENCE_MS) {
                stopExportTicker();
                showToast('error', {
                    text: 'Export stopped responding. Nothing was saved — try again.',
                    duration: 8000
                });
                return;
            }
    ...
    // Ask the extension how the export is really doing, rather than waiting to
    // be told. This keeps the service worker awake, and it is what notices an
    // exporter that has stopped existing instead of one that is merely slow.
    function startExportPolling() {
        if (exportPoller) return;
        exportPoller = setInterval(() => {
            try {
                chrome.runtime.sendMessage({ type: 'EXPORT_POLL' }, (reply) => {
                    if (chrome.runtime.lastError) return;
                    if (reply && reply.running === false) stopExportTicker();
                });
            } catch (_) {
    ...
    function showToast(type = 'loading', opts = {}) {
        // Old call shape: showToast(type, durationMs).
        if (typeof opts === 'number') opts = { duration: opts };
        const { text = '', duration = 0, cancellable = false } = opts;
        ...
        if (!toastEl) {
            ...
            icon.className = 'ac-toast-icon';
            toastText = document.createElement('span');
            toastText.className = 'ac-toast-text';
            toastClose = document.createElement('button');
            toastClose.type = 'button';
            toastClose.className = 'ac-toast-close';
            toastClose.title = 'Cancel export';
            toastClose.setAttribute('aria-label', 'Cancel export');
            toastClose.textContent = '✕';
            toastClose.addEventListener('mousedown', (e) => e.preventDefault()); // keep the page's selection
            toastClose.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                sendMessage({ type: 'EXPORT_CANCEL' });
                showToast('loading', { text: 'Cancelling…' });
            });

            toastEl.appendChild(icon);
            toastEl.appendChild(toastText);
            toastEl.appendChild(toastClose);
            document.documentElement.appendChild(toastEl);
        }

        clearTimeout(toastTimer);
        const icon = toastEl.firstChild;
        if (type === 'loading') {
            icon.innerHTML = '<span class="ac-toast-spinner"></span>';
        } else {
            icon.textContent = TOAST_ICONS[type] || '';
        }
        toastText.textContent = text;
        toastClose.hidden = !cancellable;
    ...
    chrome.runtime.onMessage.addListener((message) => {
        ...
        if (message.type === 'EXPORT_LOG') {
            // Put the exporter's own account of itself where it can be read.
            console.log('[Audio Cursor export]', message.text);
            return;
        }

        if (message.type === 'TRIGGER_DOWNLOAD_AUDIO') {
            const text = getSelectedText() || selectedText;
            if (text && text.trim()) {
                sendMessage({ type: 'EXPORT_MP3', text: text.trim() });
            } else {
                showToast('info', { text: 'Select some text to export it as MP3', duration: 2500 });
            }
            return;
        }

        if (message.type === 'DOWNLOAD_STATUS') {
            const voice = message.voiceName ? ' · ' + message.voiceName : '';
            switch (message.status) {
                case 'loading':
                    exportBaseText = 'Preparing export' + voice;
                    exportNote = '';
                    noteExportActivity();
                    renderExportToast();
                    break;
                case 'progress':
                    // `done` is null on a note: keep the count already showing.
                    if (typeof message.done === 'number') {
                        exportBaseText = `Exporting ${message.done} / ${message.total}${voice}`;
                        exportNote = '';
                    }
                    if (message.note) exportNote = message.note;
                    noteExportActivity();
                    renderExportToast();
                    break;
                case 'saving':
                    exportBaseText = 'Saving MP3';
                    exportNote = '';
                    noteExportActivity();
                    renderExportToast();
                    break;
                case 'success':
                    stopExportTicker();
                    showToast(message.skipped ? 'error' : 'success', {
                        text: message.skipped
                            ? `Saved ${message.filename || 'MP3'} — ${message.skipped} part${message.skipped > 1 ? 's' : ''} could not be read`
                            : `Saved ${message.filename || 'MP3'}`,
                        duration: message.skipped ? 8000 : 4000
                    });
                    break;
                case 'error':
                    stopExportTicker();
                    showToast('error', {
                        text: message.error ? `Export failed: ${message.error}` : 'Export failed',
                        duration: 7000
                    });
                    break;
                case 'cancelled':
                    stopExportTicker();
                    showToast('info', { text: 'Export cancelled', duration: 2000 });
                    break;
                case 'busy':
                    showToast('info', { text: 'Already exporting — wait for it to finish, or press ✕ to cancel', duration: 2500 });
                    break;
                default:
                    hideToast();
            }
