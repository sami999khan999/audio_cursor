...
- [x] **A1** — `vscode-ext/package.json`: `name`, `displayName: "Audio Cursor"`, `description`, `version: 0.1.0`, `publisher`, `engines.vscode`, `categories: ["Other", "Accessibility"]`, `main: "./src/extension.js"`.
- [x] **A2** — Add `activationEvents: ["onStartupFinished"]` (needed so the status bar and selection listeners exist before any command runs).
- [x] **A3** — Add `contributes.viewsContainers.activitybar`: id `audioCursor`, title `Audio Cursor`, icon `media/icon.svg`.
- [x] **A4** — Add `contributes.views.audioCursor`: one entry, id `audioCursor.player`, name `Player`, `"type": "webview"`.
- [x] **A5** — Create `media/icon.svg` — 24×24, single path, `fill="currentColor"`, no hardcoded color (VS Code recolors it per theme/state).
- [x] **A6** — `jsconfig.json` with `checkJs: false`, `"types": ["vscode"]`; install `@types/vscode` and `@vscode/vsce` as devDependencies.
- [x] **A7** — `.vscodeignore` excluding `spike/`, `.vscode/`, `jsconfig.json`, `*.md` except README/CHANGELOG.
- [x] **A8** — `.vscode/launch.json` with an Extension Development Host config so `F5` works.

### 1b. Bootstrap code

- [x] **A9** — `src/log.js`: create an `OutputChannel("Audio Cursor")`; export `info/warn/error`. Every module logs through this — no `console.log`.
- [x] **A10** — `src/extension.js`: `activate(context)` logs "activated" and pushes disposables; `deactivate()` is a no-op stub.
- [x] **A11** — `src/view/provider.js`: a `WebviewViewProvider` registered with `retainContextWhenHidden: true` and `enableScripts: true`; `localResourceRoots` limited to `src/view` and `media`.
- [x] **A12** — `src/view/player.html` with a strict CSP meta tag (`default-src 'none'; style-src ${cspSource}; script-src 'nonce-${nonce}'`) and a nonce generator in `provider.js`.
- [x] **A13** — Render a "Hello" placeholder in the view. **Done when:** `F5` opens a dev host, the Audio Cursor icon appears in the activity bar, and clicking it shows the placeholder.
